...
```{r delay-example0, fig.height=2, fig.width=7}
...
```{r delay-example1, fig.height=2, fig.width=7}
delay_graph                                      +
...
```{r delay-example2, fig.height=2, fig.width=7}
delay_graph                                      +
...
```{r delay3, fig.height=4, fig.width=4}
f <- "6-beta-test2.jags"
...
pts.m <- jags.model(f, data=dat_before, 
  n.adapt = 100)

parms <- c("lambda", "delta2", "delta3", "Nstar")
pts.t <- coda.samples(pts.m, var=parms, n.iter = 1000)
summary(pts.t)
pts.matrix <- as.matrix(pts.t)
nm <- dimnames(pts.matrix)[[2]]
hist(pts.matrix[, "Nstar"], breaks=50, main=nm[1])
hist(pts.matrix[, "delta2"], breaks=50, main=nm[2])
hist(pts.matrix[, "delta3"]*1095, breaks=50, main=nm[2])
hist(pts.matrix[, "lambda"]*30, breaks=50, main=nm[3])
```

```{r delay4, fig.height=4, fig.width=4}
f <- "6-beta-test2.jags"
pts <- as.numeric(unlist(read.csv("2-paths-count.csv")))
dat_during <- list(
  N=350, T=3*365, S=0.5, 
  D2=0.4,  S2=0.1,
  D3=0.17, S3=0.2,
...
pts.x <- jags.model(f, data=dat_during, 
  n.adapt = 100)

parms <- c("lambda", "delta2", "delta3", "Nstar")
pts.t <- coda.samples(pts.x, var=parms, n.iter = 1000)
summary(pts.t)
pts.matrix <- as.matrix(pts.t)
nm <- dimnames(pts.matrix)[[2]]
hist(pts.matrix[, "Nstar"], breaks=50, main=nm[1])
...
```{r delay0, fig.height=4, fig.width=4}
f <- "6-beta-test1.jags"
# Here's what's hiding in the file 
cat(readLines(f), sep="\n")
dat_before <- list(N=350, T=3*365, S=0.5, D1=0.1, S1=0.2, t=0, n=0)
...
nm <- dimnames(pts.matrix)[[2]]
hist(pts.matrix[, "Nstar"], breaks=50, main=nm[1])
hist(pts.matrix[, "delta1"]*1095, breaks=50, main=nm[2])
hist(pts.matrix[, "lambda"]*30, breaks=50, main=nm[3])
```

```{r delay1, fig.height=4, fig.width=4}
f <- "6-beta-test1.jags"
pts <- rep(0, 150)
dat_during <- list(N=350, T=3*365, S=0.5, D1=0.1, S1=0.2, t=150, n=pts)

pts.x <- jags.model(f, data=dat_during, 
  n.adapt = 100)

parms <- c("lambda", "delta1", "Nstar")
pts.t <- coda.samples(pts.x, var=parms, n.iter = 1000)
summary(pts.t)
pts.matrix <- as.matrix(pts.t)
nm <- dimnames(pts.matrix)[[2]]
...
```{r delay2, fig.height=4, fig.width=4}
f <- "6-beta-test1.jags"
pts <- as.numeric(c(rep(0, 150), unlist(read.csv("2-paths-count.csv"))))
dat_during <- list(N=350, T=3*365, S=0.5, D1=0.1, S1=0.2, t=length(pts), n=pts)
...
```{r delay-during, fig.width=4, fig.height=4, eval=FALSE}
pts <- as.numeric(c(rep(0, 58), unlist(read.csv("2-paths-count.csv"))))
...
pts.m <- jags.model("6-beta-extension.jags", data=data, inits=inits,
  n.adapt = 100)
