...
S1 <- 0.1
y_label1 <- "Days before start (prior, S1=0.1)"
sim1 <- data.frame(x=" ", y=rbeta(n_reps, S1*N*D1, S1*N*(1-D1))*T)
S1 <- 0.2
y_label2 <- "Days before start (prior, S1=0.2)"
sim2 <- data.frame(x=" ", y=rbeta(n_reps, S1*N*D1, S1*N*(1-D1))*T)
S1 <- 0.5
y_label3 <- "Days before start (prior, S1=0.5)"
sim3 <- data.frame(x=" ", y=rbeta(n_reps, S1*N*D1, S1*N*(1-D1))*T)

y_range <- range(sim1$y, sim2$y, sim3$y)
custom_boxplot(sim1, y_label1, 0, "gray") + expand_limits(y=y_range)
custom_boxplot(sim2, y_label2, 0)         + expand_limits(y=y_range)
custom_boxplot(sim3, y_label3, 0, "gray") + expand_limits(y=y_range)
...
S2 <- 0.1
y_label1 <- "Monthly accrual rate during early phase (prior, S2=0.1)"
sim1 <- data.frame(x=" ", y=rbeta(n_reps, S2*N*D2, S2*N*(1-D2))*N/T*30)
S2 <- 0.2
y_label2 <- "Monthly accrual rate during early phase (prior, S2=0.2)"
sim2 <- data.frame(x=" ", y=rbeta(n_reps, S2*N*D2, S2*N*(1-D2))*N/T*30)
S2 <- 0.5
y_label3 <- "Monthly accrual rate during early phase (prior, S2=0.5)"
sim3 <- data.frame(x=" ", y=rbeta(n_reps, S2*N*D2, S2*N*(1-D2))*N/T*30)

y_range <- range(sim1$y, sim2$y, sim3$y)
custom_boxplot(sim1, y_label1, 0)         + expand_limits(y=y_range)
custom_boxplot(sim2, y_label2, 0, "gray") + expand_limits(y=y_range)
custom_boxplot(sim3, y_label3, 0, "gray") + expand_limits(y=y_range)
```
...
S3 <- 0.1
y_label1 <- "Days in warmup (prior, S3=0.1)"
sim1 <- data.frame(x=" ", y=rbeta(n_reps, S3*N*D3, S3*N*(1-D3))*T)
S3 <- 0.2
y_label2 <- "Days in warmup (prior, S3=0.2)"
sim2 <- data.frame(x=" ", y=rbeta(n_reps, S3*N*D3, S3*N*(1-D3))*T)
S3 <- 0.5
y_label3 <- "Days in warmup (prior, S3=0.5)"
...
mod_dm0 <- jags.model(f, data=dat_before, 
  n.adapt = 100)

parms <- c("delta2", "delta3", "lambda", "Nstar")
