...
### Figure 6.5. Prior distribution for delta1, rescaled to number of days.
...
custom_boxplot(sim1, d1_label0a, 0, "gray") + expand_limits(y=y_range)
custom_boxplot(sim2, d1_label0b, 0)         + expand_limits(y=y_range)
custom_boxplot(sim3, d1_label0c, 0, "gray") + expand_limits(y=y_range)
```
...
### Figure 6.6. Prior distribution for delta2, rescaled to patients per month.
...
custom_boxplot(sim1, d2_label0a, 1, "gray") + expand_limits(y=y_range)
custom_boxplot(sim2, d2_label0b, 1)         + expand_limits(y=y_range)
custom_boxplot(sim3, d2_label0c, 1, "gray") + expand_limits(y=y_range)
```
...
### Figure 6.7. Prior distribution for delta3, rescaled to days.
...
custom_boxplot(sim1, d3_label0a, 0, "gray") + expand_limits(y=y_range)
custom_boxplot(sim2, d3_label0b, 0)         + expand_limits(y=y_range)
custom_boxplot(sim3, d3_label0c, 0, "gray") + expand_limits(y=y_range)
```
...
### Figure 6.8. Effect of prior parameters on estimated total sample size.

```{r scaterplots, fig.width=4, fig.height=4}
sim_dm0                                         %>%
  mutate(x=delta1*T)                            %>%
  mutate(y=Nstar)                               %>%
  custom_scatterplot(d1_label0, N_label0, round_x=0)
sim_dm0                                         %>%
  mutate(x=30*delta2*N/T)                       %>%
  mutate(y=Nstar)                               %>%
  custom_scatterplot(d2_label0, N_label0)
sim_dm0                                         %>%
  mutate(x=delta3*T)                            %>%
  mutate(y=Nstar)                               %>%
  custom_scatterplot(d3_label0, N_label0, round_x=0)
sim_dm0                                         %>%
  mutate(x=lambda*30)                           %>%
  mutate(y=Nstar)                               %>%
  custom_scatterplot(l_label0, N_label0, round_x=0)
```
...
### Updating the delay simulation--`r dat_update1$t` days of nothing.
...
### Figure 6.9. Updated estimate of total sample size after `r dat_update1$t` days of nothing.
...
### Figure 6.10. How much longer do you expect to wait after 150 days of nothing.

```{r updated_start, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=delta1*T)                            %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d1_label0, 0, co="gray")       -> delta10
sim_dm1                                         %>%
  mutate(y=delta1*T)                            %>%
  mutate(x=" ")                                 %>%
...
### Figure 6.11. Update of delta2 after 150 days of nothing.

```{r remaining_parameters, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=delta2*30*N/T)                       %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d2_label0, 1, co="gray")       -> delta20
sim_dm1                                         %>%
  mutate(y=delta2*30*N/T)                       %>%
  mutate(x=" ")                                 %>%
...
### Figure 6.12. Update of delta3 after 150 days of nothing.

```{r update_delta3, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=delta3*T)                            %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d3_label0, 0, co="gray")       -> delta30
sim_dm1                                         %>%
  mutate(y=delta3*T)                            %>%
  mutate(x=" ")                                 %>%
...
### Figure 6.12. Updated of lambda after 150 days of nothing.

```{r updated_lambda, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=lambda*30)                           %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(l_label0, 1, co="gray")        -> lambda0
sim_dm1                                         %>%
  mutate(y=lambda*30)                           %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(l_label1, 1)                   -> lambda1
r <- range(sim_dm0$lambda, sim_dm1$lambda)*30
lambda0 + expand_limits(y=r)
lambda1 + expand_limits(y=r)
```

### Update 2. We have 240 days of accrual data after 192 days of waiting.

On day 192 (finally!) the trial starts accruing patients. The early phase, as expected, was slow, but now you have `r sum(dat_update2$n)` patients after `r t` days of active accrual. So how are we doing now?

### Figure 6.13. Second update of total sample size.
...
sim_dm1                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(N_label1, 0, co="gray")        -> update1
sim_dm0                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(N_label2, 0, co="gray")        -> prior_estimate
...
Figure 6.14. Second update of delta1.
...
delta10 + expand_limits(y=r)
delta11 + expand_limits(y=r)
delta12 + expand_limits(y=r)
```

### Figure 6.15. Second update of delta2.

```{r shortfall2, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=delta2*30*N/T)                       %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d2_label0, 1, co="gray")       -> delta20
sim_dm1                                         %>%
  mutate(y=delta2*30*N/T)                       %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d2_label1, 1, co="gray")       -> delta21
sim_dm2                                         %>%
  mutate(y=delta2*30*N/T)                       %>%
  mutate(x=" ")                                 %>%
...
### Figure 6.16. Second update of delta3.

```{r shortfall3, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=delta3*T)                            %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d3_label0, 0, co="gray")       -> delta30
sim_dm1                                         %>%
  mutate(y=delta3*T)                            %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(d3_label1, 0, co="gray")       -> delta31
sim_dm2                                         %>%
  mutate(y=delta3*T)                            %>%
  mutate(x=" ")                                 %>%
...
### Figure 6.17. Second update of lambda.

```{r shortfall4, fig.width=7, fig.height=1}
sim_dm0                                         %>%
  mutate(y=lambda*30)                           %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(l_label0, 1, co="gray")        -> lambda0
sim_dm1                                         %>%
  mutate(y=lambda*30)                           %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(l_label1, 1, co="gray")        -> lambda1
sim_dm2                                         %>%
  mutate(y=lambda*30)                           %>%
  mutate(x=" ")                                 %>%
