...
Try a different informative prior. This prior has the same weight as before (100), but this time the prior is not too much different from the data.
...
dat <- list(n=35, t=6, N=200, T=36, S=0.5)
fit01b <- stan(file=f, data=dat)
tidy(fit01b)
```

The standard errors are much smaller with either informative prior than with a non-informative prior.
...
```{r mod02b, error=TRUE}
# Here's the data. S=1/200 gives a prior weight of one subject.
dat <- list(J=4, n=c(5, 12, 10, 8), t=6, N=200, T=36, S=0.5)
fit02b <- stan(file=f, data=dat)
tidy(fit02b)
```

These results are identical to fit01b.
...
cat(readLines(f), sep="\n")
# Here's the data.
dat <- list(J=4, n=c(5, 12, 10, 8), t=6, N=200, T=24, pseudo_n=100, pseudo_t=12, S=0.5)
...
```{r mod04b, error=TRUE}
# Here's the data.
dat <- list(J=4, n=c(5, 12, 10, 8), t=6, N=200, T=36, pseudo_n=100, pseudo_t=18, S=0.5)
