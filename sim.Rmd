...
```{r gamma_prior, fig.width=7, fig.height=1.67}
n_s <- length(s_list)
n_reps <- 1000
S <- rep(s_list, each=n_reps)
pr1 <- tibble(x=paste("S =", S), y=rgamma(n_s*n_reps, S*N, S*T/30))
...
As a researcher who has done this sort of clinical trial many times, the value of S=`r S` seems reasonable. Smaller values of S produce distributions that are much wider than what you'd expect in a trial like this. With this value of S, you are ready to simulate your clinical trial.
...
lambda_list <- c(
  ...
  qgamma(runif(1, 0.4,  0.6),  N*S, T*S),
