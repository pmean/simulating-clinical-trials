...
ggplot(df, aes(x=n, y=t, group=g)) +
  ...
  geom_label(aes(n, t, label=lb), data=total, size=2.5) +  
  coord_flip()
```
...
ggplot(df, aes(x=n, y=t, group=g)) +
  ...
  geom_vline(xintercept=350) +
  expand_limits(y=1200)      +
  geom_label(aes(n, t, label=lb[1]), data=total, size=2.5) +  
  coord_flip()
```
...
```{r gamma-poisson-boxplot1, fig.height=2}
...
```{r gamma-poisson-scatterplot1, fig.height=5}
...
```{r gamma-poisson-scatterplot2, fig.height=5}
# Scatterplot of accrual rate versus total sample size
sim_gp1                                       %>%
...
```{r gamma-poisson-during-trial, fig.height=2.5}
...
```{r plot-very-flat}
# split into two pieces to save time during testing
very_flat_predictions %>%
  ...
  ggtitle("Predictions from a flat prior") +
...
informative_predictions %>%
  ...
  geom_line(aes(x=t, y=hi)) +
  geom_line(aes(x=t, y=lo)) +
  ggtitle("Predictions from an informative prior") +
  ylab("Estimated trial duration (years)") +
  xlab("Date of prediction (days)")
```
...
```{r hedging-extension-during-trial, fig.height=3.5}
# hedging posterior
...
```{r downweight, fig.height=1.5, fig.width=7}
sim_h12 %>%
  mutate(i=" ")                          %>%
  ggplot(aes(i, pi))                      +
...
```{r hedging-extension-during-trial2, fig.height=3.5}
# hedging posterior
f <- "hedging-extension-during-trial.stan"
...
```{r downweight2, fig.height=1.5, fig.width=7}
sim_h14 %>%
  mutate(i=" ")                          %>%
  ggplot(aes(i, pi))                      +
  expand_limits(y=c(0,2))                 +
...
delay_graph +
  geom_path(aes(x, y), data=df2, color="yellow", size=3) +
  geom_path(aes(x, y), data=df2, color="gray",  size=1)
```
...
```{r delay-priors1, fig.height=1.5, fig.width=5}
S1 <- 0.2
D1 <- 0.1
N <- 350
dp1 <- data.frame(S1=S1, y=rbeta(1000, S1*N*D1, S1*N*(1-D1)))

ggplot(dp1, aes(factor(S1), y)) +
  geom_boxplot() +
  labs(x="", y="Proportion of time waiting to start") +
  coord_flip()
```

```{r delay-priors2, fig.height=1.5, fig.width=5}
S2 <- 0.1
D2 <- 0.4
dp2 <- data.frame(S2=S2, y=rbeta(1000, S2*N*D2, S2*N*(1-D2)))

ggplot(dp2, aes(factor(S2), y)) +
  geom_boxplot() +
  labs(x="", y="Accrual reduction during warm-up") +
  coord_flip()
```

```{r delay-priors3, fig.height=1.5, fig.width=5}
S3 <- 0.2
D3 <- 0.17
dp3 <- data.frame(S2=S2, y=rbeta(1000, S3*N*D3, S3*N*(1-D3)))

ggplot(dp3, aes(factor(S3), y)) +
  geom_boxplot() +
  labs(x="", y="Proportion of time in slow accrual") +
  coord_flip()
```

```{r delay-three, fig.height=1.5, fig.width=5}
...
```{r binomial, fig.height=1.5, fig.width=5}
