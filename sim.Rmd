...
```{r gamma_prior_0.2, fig.width=5, fig.height=1}
pr1 <- tibble(x="S=0.2", y=rgamma(1000, 0.2*350, 0.2*1095/30))

pr1                                             %>%
  ggplot(aes(x, y))                              +
  geom_boxplot()                                 +
  xlab("")                                       +
  ylab("Prior estimate of monthly accrual rate") +
  coord_flip()
  labs(x="", y="Prior estimate of monthly accrual rate") +
  coord_flip()
```
...
```{r gamma_prior_0.5, echo=FALSE, fig.width=5, fig.height=1.33}
pr2 <- tibble(x="S=0.5", y=rgamma(1000, 0.5*350, 0.5*1095/30))
...
The second distribution (S=0.5) seems to match your gut feeling about accrual because a monthly accrual rate is unlikely to be smaller than 7.5 patients per month or more than 12.5 patients per month.
...
```{r simulate_time, fig.width=5, fig.height=5}
...
st <- tibble(g=1, n=1:350, t=cumsum(rexp(350, lambda)))
lambda <- qgamma(runif(1, 0.4, 0.6), 0.5*350, 0.5*1095)
st <- rbind(st, tibble(g=2, n=1:350, t=cumsum(rexp(350, lambda))))
lambda <- qgamma(runif(1, 0.9, 0.99), 0.5*350, 0.5*1095)
st <- rbind(st, tibble(g=3, n=1:350, t=cumsum(rexp(350, lambda))))
t_total <- st$t[st$n==350]

ggplot(st, aes(x=n, y=t, group=g))               +
  geom_line()                                    +
  ...
  geom_label(aes(x=350), y=t_total[1], 
             label=round(t_total[1]), size=2.5)  +  
  geom_label(aes(x=350), y=t_total[2], 
             label=round(t_total[2]), size=2.5)  +  
  geom_label(aes(x=350), y=t_total[3], 
             label=round(t_total[3]), size=2.5)  + 
  coord_flip()
```
...
lambda <- qgamma(runif(1, 0.01, 0.1), 0.5*350, 0.5*1095)
sn <- tibble(g=1, t=1:1095, n=cumsum(rpois(1095, lambda)))
lambda <- qgamma(runif(1, 0.4, 0.6), 0.5*350, 0.5*1095)
sn <- rbind(sn, tibble(g=2, t=1:1095, n=cumsum(rpois(1095, lambda))))
lambda <- qgamma(runif(1, 0.9, 0.99), 0.5*350, 0.5*1095)
sn <- rbind(sn, tibble(g=3, t=1:1095, n=cumsum(rpois(1095, lambda))))
n_total <- sn$n[sn$t==1095]

ggplot(sn, aes(x=n, y=t, group=g))               +
  geom_line()                                    +
  ...
  ylab("Time (days)")                            +
  xlab("Number of patients")                     +
  geom_label(aes(x=n_total[1], y=1095, 
                 label=n_total[1]), size=2.5)    +  
  geom_label(aes(x=n_total[2], y=1095,
                 label=n_total[2]), size=2.5)    +  
  geom_label(aes(x=n_total[3], y=1095, 
                 label=n_total[3]), size=2.5)    +
  coord_flip()
```
...
lambda <- qgamma(runif(1, 0.01, 0.1), 0.5*350, 0.5*1095)
sb <- tibble(g=1, t=1:1095, n=cumsum(rpois(1095, lambda)))
lambda <- qgamma(runif(1, 0.4, 0.6), 0.5*350, 0.5*1095)
sb <- rbind(sb, tibble(g=2, t=1:1095, n=cumsum(rpois(1095, lambda))))
lambda <- qgamma(runif(1, 0.9, 0.99), 0.5*350, 0.5*1095)
sb <- rbind(sb, tibble(g=3, t=1:1095, n=cumsum(rpois(1095, lambda))))
sb$n[sb$n>350] <- 350
co <- c(FALSE, sb$n[-1]==350 & sb$n[-3*1095]==350)
sb <- sb[!co, ]
sb                                              %>% 
  group_by(g)                                   %>%
  slice(n())                                    %>%
...
```{r gamma-poisson-before-trial, fig.width=5, fig.height=1}
...
```{r gamma-poisson-boxplot1, fig.width=5, fig.height=1}
...
```{r gamma-poisson-after-trial, echo=FALSE, fig.width=5, fig.height=1.33}
data.frame(
  i="after",
  lambda=341/ 1336,
...
for (i in seq(1, nx, by=1)) {
  ...
  qx <- quantile(waiting_time, probs=c(0.025, 0.975))
...
fit_h12                                         %>%
  as.data.frame                                 %>%
  mutate(i="hedging posterior")                 -> sim_h12
...
fit_gp1                                         %>%
  as.data.frame                                 %>%
  mutate(i="informative prior")                 -> sim_gp1
...
fit_gp2                                         %>%
  as.data.frame                                 %>%
  mutate(i="informative posterior")             -> sim_gp2

# flat posterior
f <- "gamma-poisson.stan"
dat_during <- list(N=350, T=3*365, S=0.001/N, n=41, t=239)
fit_fl2 <- stan(file=f,
  data=dat_during, iter= 1000, chains = 4)
fit_fl2                                         %>%
  as.data.frame                                 %>%
  mutate(i="flat posterior") -> sim_fl2

# data
sim_dat <- data.frame(lambda=41/239, Nstar=41+(1095-239)*(41/239), i="Data")
```

```{r hedging-boxplots, fig.width=5, fig.height=2.67}
# Boxplot of total sample size
sim_h12                                         %>%
  bind_rows(sim_gp1)                            %>%
  bind_rows(sim_gp2)                            %>%
...
```{r hedging-extension2, fig.height=2.67}
# hedging posterior
f <- "hedging-extension.stan"
...
fit_h14                                         %>%
  as.data.frame                                 %>%
  mutate(i="hedging posterior")                 -> sim_h14
...
# flat posterior
f <- "gamma-poisson.stan"
dat_during <- list(N=350, T=3*365, S=0.001/N, n=41, t=129)
fit_fl3 <- stan(file=f,
  data=dat_during, iter= 1000, chains = 4)
fit_fl3                                         %>%
  as.data.frame                                 %>%
...
```{r hedging-boxplot2, fig.width=5, fig.height=2.6}
# Boxplot of total sample size
sim_h14                                         %>%
  bind_rows(sim_gp4)                            %>%
  bind_rows(sim_gp5)                            %>%
...
```{r downweight2, fig.height=1, fig.width=5}
sim_h14 %>%
  mutate(i=" ")                          %>%
  ggplot(aes(i, pi))                      +
  expand_limits(y=c(0,2))                 +
...
```{r delay-example0, fig.height=1.5, fig.width=5}
...
```{r delay-example1, fig.height=1.5, fig.width=5}
delay_graph                                      +
...
```{r delay-example2, fig.height=1.5, fig.width=5}
delay_graph                                      +
...
```{r delay-priors1, fig.height=1, fig.width=5}
N=350
T=3*365
S=0.5
S1 <- 0.2
D1 <- 0.1
N <- 350
dp1 <- data.frame(S1=S1,
  delta1=rbeta(1000, S1*N*D1, S1*N*(1-D1)))
summary(dp1)
dp1                                             %>%
  ggplot(aes(factor(S1), delta1))                +
  geom_boxplot()                                 +
  xlab(" ")                                      + 
  ylab("Proportion of time waiting to start")    +
  coord_flip()

# units conversion
dp1                                             %>%
  mutate(delta1=T*delta1)                       %>%
  ggplot(aes(factor(S1), delta1))                +
  geom_boxplot()                                 +
  xlab(" ")                                      + 
  ylab("Days waiting to start")                  +
  coord_flip()
```
...
```{r delay-priors2, fig.height=1, fig.width=5}
N <- 350
T <- 3*365
S <- 0.5
S2 <- 0.1
...
dp2 <- data.frame(S2=S2,
  delta2=rbeta(1000, S2*N*D2, S2*N*(1-D2)))
summary(dp2)
...
dp2                                             %>%
  ...
  geom_boxplot()                                 +
  xlab(" ")                                      +
  ylab("Accrual reduction during warm-up")       +
  coord_flip()
...
```{r delay-priors3, fig.height=1, fig.width=5}
N <- 350
T <- 3*365
S <- 0.5
S3 <- 0.2
D3 <- 0.17
dp3 <- data.frame(S3=S3, delta3=rbeta(1000, S3*N*D3, S3*N*(1-D3)))
summary(dp3)
dp3                                             %>%
  ggplot(aes(factor(S3), delta3))                +
  geom_boxplot()                                 +
  xlab(" ")                                      +
  ylab("Proportion of time in slow accrual")     +
  coord_flip()

# units conversion
dp3                                             %>%
  mutate(delta3=T*delta3)                       %>%
  ggplot(aes(factor(S3), delta3))                +
  geom_boxplot()                                 +
  xlab(" ")                                      +
  ylab("Days in slow accrual")                   +
  coord_flip()
```
...
```{r delay-three, fig.height=1, fig.width=5}
...
```{r delay-part2, fig.width=5, fig.height=5}
sim_d31                                         %>%
...
pts <- c(rep(0, 58),unlist(read.csv("count.csv")))
...
parms <- c("lambda", "delta2", "delta3", "Nstar")
pts.samp <- coda.samples(pts.m, var=parms, n.iter = 1000)
summary(pts.samp)
pts.matrix <- as.matrix(pts.samp)
nm <- dimnames(pts.matrix)[[2]]
hist(pts.matrix[, 1], breaks=50, main=nm[1])
hist(30*pts.matrix[, 2]*pts.matrix[, 4], breaks=50, main=nm[2])
hist(1095*pts.matrix[, 3], breaks=50, main=nm[3])
hist(30*pts.matrix[, 4], breaks=50, main=nm[4])
```
