...
Third, a strong prior distributiton provides you with stable estimates early in the clinical trial. This is worth an illustration. There is a well known formula for an exact confidence interval for a Poisson rate that works with counts as low as 1. From that confidence interval on the Poisson rate, you can extrapolate to the total amount of time that the trial will take. Let's apply this formula from day 1 of the clinical trial.


```{r paths-using-no-prior}
pts <- unlist(read.csv("count.csv"))
# ( qchisq(0.025, 2*x)/2, qchisq(0.975, 2*(x+1))/2 )
x <- cumsum(pts)
lo <- (qchisq(0.025, 2*x)    /2) / (1:length(x))
hi <- (qchisq(0.975, 2*(x+1))/2) / (1:length(x))
lo <- ((1:length(x)) + (350-x)/ lo) / 365
hi <- ((1:length(x)) + (350-x)/ hi) / 365
data.frame(t=1:length(x), lo=lo, hi=hi) %>%
  ggplot(aes(t,hi)) +
  geom_ribbon(aes(ymin=lo, ymax=hi), alpha=0.2) +
  geom_line(aes(x=t, y=hi)) +
  geom_line(aes(x=t, y=lo)) +
  ylab("Estimated trial duration (years)") +
  xlab("Date of prediction (days)") -> no_prior
print(no_prior)
```

This is possibly an unfair example because the trial had a slow start. But it takes more than 100 days into the trial to get a decent estimate of the upper bound. The first three predictions are a measure of how unstable things are. On the first day, you have recruited one patient, and the upper limit is estimated at `r round(lo[1], 1)` years. On the second day, no one shows up, so your upper limit jumps to `r round(lo[2], 1)` years. On the third day, one patient shows up, so your upper limit plummets to `r round(lo[3], 1)` years.
...
pts <- unlist(read.csv("count.csv"))
x <- cumsum(pts)
f <- "gamma-exponential-during-trial.stan"
nx <- length(x)
informative_predictions <- data.frame(t <- 1:nx, lo=rep(0,nx), hi=rep(0,nx))
for (i in seq(1, nx, by=1)) {
  dat_during <- list(N=350, T=3*365, S=0.5, n=x[i], t=i)
  fit_gp0 <- stan(file=f,
    data=dat_during, iter= 1000, chains = 4)
  sum_gp0 <- as.data.frame(fit_gp0)
  qx <- quantile(sum_gp0$Tstar, probs=c(0.025, 0.975))
  informative_predictions[i, 2:3] <- qx
  # if (i==1) {informative_predictions <}
  print(c(i,qx))
}
```
...
no_prior +
  geom_ribbon(aes(x=t, ymin=lo, ymax=hi),
    data=informative_predictions, alpha=0.2) +
  geom_line(aes(x=t, y=hi)) +
  geom_line(aes(x=t, y=lo)) 
informative_predictions %>%
...
for (i in seq(1, nx, by=1)) {
  dat_during <- list(N=350, T=3*365, S=1/350, n=x[i], t=i)
  fit_gp0 <- stan(file=f,
    data=dat_during, iter= 1000, chains = 4)
  sum_gp0 <- as.data.frame(fit_gp0)
  qx <- quantile(sum_gp0$Tstar, probs=c(0.025, 0.975))
