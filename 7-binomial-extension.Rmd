...
custom_boxplus(sim1, b1_label0a, 2, "gray") + expand_limits(y=y_range)
custom_boxplus(sim2, b1_label0b, 2)         + expand_limits(y=y_range)
custom_boxplus(sim3, b1_label0c, 2, "gray") + expand_limits(y=y_range)
```
...
prior1_plot + expand_limits(y=y_range)
prior2_plot + expand_limits(y=y_range)
prior3_plot + expand_limits(y=y_range)
```
...
y_range <- range(sim_bi1$y, sim_bi5$y)
prior1_plot  + expand_limits(y=y_range)
update1_plot + expand_limits(y=y_range)
```

### Figure 7.6.

```{r tm2, fig.width=7, fig.height=1}
y_range <- range(sim_bi2$y, sim_bi6$y)
prior2_plot  + expand_limits(y=y_range)
update2_plot + expand_limits(y=y_range)
```

### Figure 7.7. Updated estimates after exclusions and refusals

```{r tm3, fig.width=7, fig.height=1}
y_range <- range(sim_bi3$y, sim_bi7$y)
prior3_plot  + expand_limits(y=y_range)
update3_plot + expand_limits(y=y_range)
```

The sample size estimate after refusals and exclusions is lower than what you were expecting prior to data collection, mostly because the proportion of refusals is a lot worse.
