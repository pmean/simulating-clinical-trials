...
l_label1 <- sub("prior", "updated", l_label0)

N_label0 <- "Estimated total sample size (prior)"
N_label1 <- sub("prior", "updated", N_label0)
```

### Part 3. Gamma Poisson models

From this point onward, you'll summarizations of the simulations using boxplots and scatterplots, but it's worth remembering that what you are actually simulating is random paths.

Let's also focus on simulations of the total sample size. In a Bayesian perspective, this is a Gamma Poisson model. The Poisson distribution models patient counts. If you place an informative gamma prior on the Poisson rate, that keeps and a Gamma distribution restricts the rate parameter of the Poisson to a reasonable range.

Try setting a distribution on the monthly accrual rate that has a mean equal to the expected accrual rate  but with varying levels of precision. 

### Figure 3.1. Boxplot of prior distribution for accrual rate.

```{r gamma_prior, fig.width=7, fig.height=1}
l_label0a <- sub("prior", paste("prior, S", S/2, sep="="), l_label0)
l_label0b <- sub("prior", paste("prior, S", S,   sep="="), l_label0)
l_label0c <- sub("prior", paste("prior, S", S*2, sep="="), l_label0)

sim1 <- data.frame(x=" ", y=rgamma(n_reps, N*S/2, T*S/2)*30)
sim2 <- data.frame(x=" ", y=rgamma(n_reps, N*S,   T*S  )*30)
sim3 <- data.frame(x=" ", y=rgamma(n_reps, N*S*2, T*S*2)*30)

y_range <- range(sim1$y, sim2$y, sim3$y)

custom_boxplot(sim1, l_label0a, 1, "gray") + expand_limits(y=y_range)
custom_boxplot(sim2, l_label0b, 1)         + expand_limits(y=y_range)
custom_boxplot(sim3, l_label0c, 1, "gray") + expand_limits(y=y_range)
```
...
sim_gp1                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(N_label0, 1)                   -> prior_estimate
prior_estimate
```
...
custom_boxplot(sim3, N_label0, 1) + expand_limits(y=y_range)
...
```{r percentile-plot1, fig.width=7, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
...
```{r percentile-plot2, fig.width=7, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
...
```{r percentile-plot3, fig.width=7, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
