...
custom_boxplot(sim1, l_label0a, 1, "black") + expand_limits(y=y_range) -> figa
custom_boxplot(sim2, l_label0b, 1, "red")   + expand_limits(y=y_range) -> figb
custom_boxplot(sim3, l_label0c, 1, "black") + expand_limits(y=y_range) -> figc
save(figa, figb, figc, file="fig/3.1.RData")
```
...
sim_sliced                                      %>%
  ...
  scale_y_continuous(breaks=y_ticks,
                     minor_breaks=NULL,
                     labels=y_labels)            +
  scale_color_manual(values=c("black", "red"))  -> fig
...
sim_gp1                                         %>%
  ...
  expand_limits(y=y_range)                      -> figa
y_label <- "Estimated total sample size (updated)"
sim_gp2                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=" ")                                 %>%
  custom_boxplot(y_label, 1, co="red")           +
  expand_limits(y=y_range)                      -> figb
save(figa, figb, file="fig/3.5.RData")
```
...
custom_boxplot(sim2, N_label0, 1, co="black") + expand_limits(y=y_range) -> figa
custom_boxplot(sim4, y_label4, 1, co="red")   + expand_limits(y=y_range) -> figb
save(figa, figb, file="fig/3.6.RData")
```
