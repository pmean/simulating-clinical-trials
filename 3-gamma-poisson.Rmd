...
```{r gamma_prior, fig.width=10, fig.height=1}
...
custom_boxplot(sim1, l_label0a, 1, "black") + expand_limits(y=y_range)
custom_boxplot(sim2, l_label0b, 1, "red")   + expand_limits(y=y_range)
custom_boxplot(sim3, l_label0c, 1, "black") + expand_limits(y=y_range)
```
...
```{r gamma-poisson-before-trial, fig.width=10, fig.height=1}
...
```{r gamma-poisson-boxplot1, fig.width=10, fig.height=1}
sim_gp1                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=" ")                                 %>%
...
```{r gamma-poisson-scatterplot1, fig.width=10, fig.height=5}
...
```{r gamma-poisson-scatterplot2, fig.width=10, fig.height=5}
# Scatterplot of accrual rate versus total sample size
sim_gp1                                         %>%
  rename(y=Nstar)                               %>%
  mutate(x=30*lambda)                           %>%
...
```{r gamma-poisson-during-trial, fig.width=10, fig.height=1}
f <- "3-gamma-poisson.stan"
...
```{r gamma-poisson-after-trial, echo=FALSE, fig.width=10, fig.height=1}
...
y_range <- range(sim2$y, sim4$y)

# Boxplot of accrual rate
custom_boxplot(sim2, N_label0, 1, co="black") + expand_limits(y=y_range)
custom_boxplot(sim4, y_label4, 1, co="red")   + expand_limits(y=y_range)
```
...
```{r percentile-plot1, fig.width=10, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
...
```{r percentile-plot2, fig.width=10, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
...
```{r percentile-plot3, fig.width=10, fig.height=1}
rgamma(12, N*S, T*S)                            %>%
